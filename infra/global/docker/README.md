...
| File | Purpose |
| --- | --- |
| `Dockerfile` | Multi-stage build for the optional backend service (a minimal REST API — swap in whatever stack you actually build; this template ships no backend source, only the scaffolding) |
...
- Build context is the **repository root** — the Dockerfile expects a `backend/` (or similar) source directory that does not exist in this template yet. Add it before building, or delete `infra/global/docker/` if you never need a backend.
